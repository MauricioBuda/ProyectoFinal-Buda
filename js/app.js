...
//Con fetch accedo a mi .json con los productos, y lo transformo a un array de objetos
//Y despues en un .then meto todas las funciones que antiguamente accedian a mi array de objetos
fetch("https://raw.githubusercontent.com/MauricioBuda/ProyectoFinal-Buda/master/js/prod.json")
.then((resp) => resp.json())
.then((data) => {
    ...
    if (producto.tipo === "ACTIVIDAD") {
        //Con este if else, le doy clases distintas a los divs segun su tipo
    ...
    const agregarAlCarrito = (prodId) => {
      //Recibo como parametro el ID del elemento segun el click que hicieron
    ...
    while (vaciarContenedorProductosParaFiltrar.firstChild) {
        vaciarContenedorProductosParaFiltrar.removeChild(
        vaciarContenedorProductosParaFiltrar.firstChild
        );
    }
    ...
    const divsFiltrados = data.filter(
        (element) => element.tipo === respuestaFiltro
    );
...
.catch((err) => console.log("Hay un error!", err)); //Recien aca termina el fetch
...
carrito.forEach((elem) => {
    //Recorro el carrito y reinicio la cantidad de cada elemento (para la ventana modal)
    elem.cantidad = 1;
});
carrito.length = 0;
...
//Recibo el ID que me da el usuario con el click
...
const paraPonerManito = (precioTotal.innerText = carrito
    .reduce((acc, prod) => acc + prod.cantidad * prod.valor, 0)
    .toFixed(2));
